...
pass-var(W, [X|Y], Z) :- check-min(W,Y,X,Z).
check-min(W, Y, X, Z) :- not(number(X)), pass-var(W,Y,Z).
check-min(X, Y, Z) :- not(length(X,0)).
min-above-min(L1, L2, N) :- is_list(L1), is_list(L2), check-min(L1,L2,N).